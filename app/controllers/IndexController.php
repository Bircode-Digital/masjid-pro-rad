...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$db->where("masjid_id", $username)->orWhere("email", $username);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("nama","email","password","foto","role"); //registration fields
		...
		$db->rawQuery("INSERT INTO data_masjid (id) VALUES ('$rec_id')");
$db->rawQuery("UPDATE users SET masjid_id='$rec_id' WHERE id='$rec_id' ");
		...
		$page_title = $this->view->page_title = "Tambah User";
