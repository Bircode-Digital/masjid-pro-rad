...
<html>
	<head>
		...
		?>
				<?php 
			Html ::  page_css('bootstrap-theme-daydream.css');
			Html ::  page_css('custom-style.css');
			Html ::  page_css('custom_index.css');
		?>
		<?php
		...
		<script>
			...
			function activateNavItem(element) {
				document.querySelectorAll('.nav-item').forEach(item => {
					...
					item.querySelector('.nav-icon').style.fill = '#666';
				});
				
				element.classList.add('active');
				element.querySelector('.nav-icon').style.fill = '#0C4B33';
			}
